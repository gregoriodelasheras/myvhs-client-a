...
@Component({
  selector: 'app-user-towatch',
  templateUrl: './user-towatch.component.html',
  styleUrls: ['./user-towatch.component.scss'],
})
export class UserTowatchComponent implements OnInit {
  // Store the to-watch movies returned by the API call.
  movies: any[] = [];
  // Set user's username.
  ...
  /**
   * Fetch all movies in user's To-Watch list
   * @returns All movies stored in the user's To-Watch list
   */
  getToWatchMovies(): void {
    this.fetchApiData.getToWatch(this.username).subscribe((resp: any) => {
      const toWatchMovies = resp.toWatchMovies;

      toWatchMovies.forEach((toWatchMovie: any) => {
        this.fetchApiData.getMovie(toWatchMovie).subscribe((resp: any) => {
